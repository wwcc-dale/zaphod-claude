...
Usage:
    ...
    python export_cartridge.py --watch-mode     # Auto-export after sync completes
    python export_cartridge.py --offline        # Export without Canvas connection
...
def generate_timestamp_filename(base_name: str = "export") -> str:
    """
    Generate a timestamped filename for export cartridge.

    Format: YYYYMMDD_HHMMSS_export.imscc
    Example: 20260204_153045_export.imscc
    """
...
def main():
    ...
    parser.add_argument(
        "--watch-mode",
        action="store_true",
        help="Enable watch mode - automatically export after sync completes"
    )
    parser.add_argument(
        "--offline",
        action="store_true",
        help="Offline mode - create export cartridge using only local files (no Canvas connection required)"
    )
    args = parser.parse_args()

    # Watch mode and offline mode are compatible - watch mode just means
    # "export after sync" while offline means "don't require Canvas"
    if args.watch_mode:
        print("[cartridge] Running in watch mode - will export after sync completes")

    if args.offline:
        print("[cartridge] Running in offline mode - using local files only")
    ...
    if not output_path:
        # Default to _course_metadata/exports/ with timestamped filename
        EXPORTS_DIR.mkdir(parents=True, exist_ok=True)
        timestamp_filename = generate_timestamp_filename()
        output_path = EXPORTS_DIR / timestamp_filename
    else:
    ...
    if args.watch_mode:
        print(f"\n[cartridge] Watch mode export complete")
        print(f"[cartridge] Ready for next sync cycle")
