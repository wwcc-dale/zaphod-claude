...
def load_bank_mappings() -> Dict[str, int]:
    """
    Load filename→bank_id mappings from bank-mappings.yaml.

    This file is generated by bank_scrape.py (HTML scrape) or sync_banks.py.
    It allows quiz frontmatter to reference banks by filename (e.g. 'bank: s1-foo.bank')
    without needing to pre-stamp bank_id values via apply_bank_ids.py.

    Returns: {filename: bank_id} e.g. {"s1-javascript-basics.bank": 35301424}
    """
    ...
    try:
        ...
        # Handle nested format: {banks: {name: {id: ...}}}
        if 'banks' in data:
            data = {k: v['id'] if isinstance(v, dict) else v for k, v in data['banks'].items()}
...
def main():
    ...
    # Merge filename-based mappings from bank-mappings.yaml (generated by bank_scrape.py)
    # This lets quiz frontmatter use 'bank: s1-foo.bank' without pre-stamping bank_id values
    file_mappings = load_bank_mappings()
    if file_mappings:
        bank_map.update(file_mappings)
        print(f"ℹ️ Loaded {len(file_mappings)} bank ID mapping(s) from bank-mappings.yaml")

    if bank_map:
        print(f"ℹ️ {len(bank_map)} bank reference(s) available for resolution")
    else:
