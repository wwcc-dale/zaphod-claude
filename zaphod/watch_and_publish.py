...
def run_pipeline(changed_files: list[Path]):
    ...
    try:
        ...
        # Pipeline steps: two groups split around the bank-generated quiz prune.
        # Canvas auto-creates a quiz for every imported bank (QTI behaviour).
        # prune_quizzes.py removes those auto-generated quizzes before sync_quizzes
        # creates the real quiz instances linked to the banks.
        pre_steps: List[Path] = [
            SCRIPT_DIR / "frontmatter_to_meta.py",
            SCRIPT_DIR / "publish_all.py",
            SCRIPT_DIR / "sync_banks.py",        # Import question banks
        ]
        post_steps: List[Path] = [
        ...
        def run_script(script: Path, extra_args: list[str] = None):
            ...
            subprocess.run(cmd, cwd=str(COURSE_ROOT), env=env, check=False)

        for script in pre_steps:
            run_script(script)

        # Remove Canvas quizzes auto-generated by bank import (must run before sync_quizzes)
        run_script(SCRIPT_DIR / "prune_quizzes.py", ["--quizzes-only", "--apply"])

        for script in post_steps:
            run_script(script)

        # Optional prune step at the end
        ...
        if prune_script.is_file():
            ...
            if prune_assignments:
                prune_args.append("--prune-assignments")
            if prune_apply:
                prune_args.append("--apply")
            run_script(prune_script, prune_args if prune_args else None)
        else:
