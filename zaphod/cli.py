...
def sync(ctx: ZaphodContext, watch: bool, course_id: Optional[int], assets_only: bool, no_prune: bool, dry_run: bool, force_quizzes: bool, quizzes_only: bool, export_on_sync: bool):
    ...
    4. prune_quizzes - Remove Canvas quizzes auto-generated by bank import
    5. sync_quizzes - Create/update quizzes linked to banks
    6. sync_modules - Organize content into modules
    7. sync_clo - Sync learning outcomes
    8. sync_rubrics - Sync rubrics to assignments
    9. prune - Clean up orphaned content (unless --no-prune)

    Note: Canvas automatically creates a quiz for every imported question bank.
    Step 4 deletes those auto-generated quizzes so step 5 can create the real
    quiz instances that draw questions from the banks.
...
@click.option('--quizzes', is_flag=True, help='Include quiz pruning (orphan quizzes + bank-generated quizzes)')
@click.pass_obj
def prune(ctx: ZaphodContext, dry_run: bool, assignments: bool, quizzes: bool):
    """
    ...
    - Orphan quizzes and bank-generated quizzes (with --quizzes)
    - Empty modules
    ...
    Examples:
        ...
        zaphod prune --quizzes              # Include orphan quizzes
    ...
    content_args = []
    if not dry_run:
        content_args.append("--apply")
    if assignments:
        content_args.append("--prune-assignments")
    ctx.run_script("prune_canvas_content.py", args=content_args if content_args else None)

    if quizzes:
        click.echo(f"\n{SWEEP} Pruning orphan quizzes...\n")
        quiz_args = ["--quizzes-only"]
        if not dry_run:
            quiz_args.append("--apply")
        ctx.run_script("prune_quizzes.py", args=quiz_args)
